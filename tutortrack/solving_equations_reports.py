...
LEVEL_LABELS = {
    ...
    2: "L2 a(j + b) = c",
    3: "L3 aj + b = dj + e",
    4: "L4 a(j+b)+c = d(j+e)+f",
    5: "L5 Fractional equations",
}
...
# -----------------------------
# invalid-reason normalization
# -----------------------------
def normalize_invalid_reason(reason: str) -> str:
    r = str(reason or "").strip().lower()

    if not r:
        return ""

    if "blank submission" in r:
        return "blank_submission"

    if "used both side operations and full equation entry" in r:
        return "mixed_input_modes"

    if "operation parse failed" in r:
        return "operation_parse_failed"

    if "operations did not match on both sides" in r:
        return "operations_mismatch"

    if "operation produced no change" in r:
        return "no_change"

    if "full equation parse failed" in r:
        return "equation_parse_failed"

    if "did not change the current equation" in r:
        return "no_change"

    if "not equivalent to the current equation" in r:
        return "not_equivalent_to_current"

    if "equivalent but not progress" in r:
        return "equivalent_but_not_progress"
...
# load + clean
# -----------------------------
def load_solving_equations_attempts_df() -> pd.DataFrame:
    df = read_sheet_as_df("solving_equations_attempts")
    ...
    expected_cols = [
        ...
        "hints_used_so_far",
        "steps_count",
        "current_equation_before",
        "current_equation_after",
    ]
    ...
    df["invalid_reason_norm"] = df["invalid_reason"].apply(normalize_invalid_reason)
    df["level_label"] = df["level"].map(LEVEL_LABELS).fillna(df["level"].astype(str))
    df["student_label"] = df["username"].replace("", "Unknown")

    df = df.sort_values(
        ["timestamp", "username", "round_key", "question_seq", "attempt_number", "attempt_id"]
    )
    return df


def _apply_filters(df: pd.DataFrame, username: str, level_filter):
    out = df.copy()
...
def _question_title_row(first: pd.Series) -> str:
    ...
    sol = str(first.get("target_solution", "")).strip()
    if sol:
        return f"{first.get('level_label', 'Level')} • target {sol}"
...
def build_question_attempt_rollup(df_attempts: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, g in df_attempts.groupby(group_cols, dropna=False):
        ...
        parse_errors = int(
            g["invalid_reason_norm"].isin(["operation_parse_failed", "equation_parse_failed"]).sum()
        )
        no_change_steps = int((g["invalid_reason_norm"] == "no_change").sum())
        non_equivalent_steps = int((g["invalid_reason_norm"] == "not_equivalent_to_current").sum())
        op_mismatch_steps = int((g["invalid_reason_norm"] == "operations_mismatch").sum())
        blank_submissions = int((g["invalid_reason_norm"] == "blank_submission").sum())
        mixed_input_modes = int((g["invalid_reason_norm"] == "mixed_input_modes").sum())

        operation_entries = int((g["input_mode"].str.lower() == "operation").sum())
        equation_entries = int((g["input_mode"].str.lower() == "equation").sum())
        ...
        row = {
            ...
            "target_solution": first["target_solution"],
            "questions_seen": 1,
            "correct": int(correct),
            ...
            "op_mismatch_steps": op_mismatch_steps,
            "blank_submissions": blank_submissions,
            "mixed_input_modes": mixed_input_modes,
    ...
    df_q["attempts_before_success"] = np.where(
        df_q["correct"] == 1,
        df_q["attempts"] - 1,
        df_q["attempts"],
    )
...
def build_summary(
    ...
    agg = (
        ...
        .agg(
            ...
            op_mismatch_steps=("op_mismatch_steps", "mean"),
            blank_submissions=("blank_submissions", "mean"),
            mixed_input_modes=("mixed_input_modes", "mean"),
            operation_entries=("operation_entries", "mean"),
            equation_entries=("equation_entries", "mean"),
        )
        .reset_index()
    )
...
def render_heat_table(df_summary: pd.DataFrame, metric: str, summary_mode: str):
    ...
    view = df_summary.copy().rename(
        columns={
            ...
            "op_mismatch_steps": "Op Mismatch",
            "blank_submissions": "Blank Subs",
            "mixed_input_modes": "Mixed Input",
            "operation_entries": "Op Entries",
            "equation_entries": "Eq Entries",
        }
    )
    ...
    styler = (
        ...
        .set_properties(subset=["Item"], **{"text-align": "left", "white-space": "normal"})
        .set_properties(
...
def render_weakest_bar(df_summary: pd.DataFrame, username: str, summary_mode: str, min_questions: int = 2):
    ...
    if summary_mode == "Question summary":
        ...
        caption = (
            "Lowest-accuracy solving-equation levels across all students"
            if username == "All students"
            else f"Lowest-accuracy solving-equation levels for {username}"
        )

    chart_df = df[[label_col, "accuracy", "questions_seen"]].sort_values("accuracy", ascending=True).head(10)
...
def render_slowest_bar(df_summary: pd.DataFrame, username: str, summary_mode: str, min_questions: int = 2):
    ...
    if summary_mode == "Question summary":
        ...
        label_col = "level_label"
        caption = (
            "Slowest solving-equation levels across all students"
            if username == "All students"
            else f"Slowest solving-equation levels for {username}"
        )

    chart_df = df[[label_col, "avg_response_time", "questions_seen"]].sort_values(
        "avg_response_time", ascending=False
    ).head(10)
...
def render_invalid_steps_bar(df_summary: pd.DataFrame, username: str, summary_mode: str, min_questions: int = 2):
    ...
    if summary_mode == "Question summary":
        label_col = "question_title"
        caption = (
            "Most invalid steps by question type across all students"
            if username == "All students"
            else f"Most invalid steps by question type for {username}"
        )
    else:
        label_col = "level_label"
        caption = (
            "Most invalid steps by level across all students"
            if username == "All students"
            else f"Most invalid steps by level for {username}"
        )

    chart_df = df[[label_col, "invalid_steps", "questions_seen"]].sort_values(
        "invalid_steps", ascending=False
    ).head(10)
    ...
    fig.add_trace(
        go.Bar(
            ...
            textposition="outside",
            customdata=chart_df["questions_seen"],
            hovertemplate="<b>%{y}</b><br>Invalid / question: %{x:.2f}<br>Questions: %{customdata}<extra></extra>",
        )
    )
    fig.update_layout(
        title=caption,
...
def render_level_effort_bubble(df_summary: pd.DataFrame, min_questions: int = 2):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=np.clip(df["questions_seen"] * 4, 12, 50),
                color=df["accuracy"],
                colorscale="RdYlGn",
                cmin=0,
                cmax=1,
            ...
            customdata=np.stack(
                [df["questions_seen"], df["invalid_steps"], df["hints_used"]],
                axis=1,
            ),
            hovertemplate=(
                "<b>%{text}</b>"
                "<br>Avg Attempts: %{x:.2f}"
                "<br>Avg Time: %{y:.2f}s"
    ...
    fig.update_layout(
        title="Level Effort Profile",
        height=520,
        xaxis=dict(title="Average Attempts per Question"),
        yaxis=dict(title="Average Response Time (seconds)"),
...
def render_accuracy_speed_scatter(df_summary: pd.DataFrame, summary_mode: str):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=np.clip(df["questions_seen"] * 2.5, 10, 34),
                color=df["avg_attempts"],
                colorscale="Blues",
                showscale=True,
                colorbar=dict(title="Avg attempts"),
                line=dict(width=1, color="black"),
            ),
            hovertemplate=(
                "<b>%{text}</b>"
    ...
    fig.update_layout(
        ...
        height=520,
        xaxis=dict(title="Average Response Time (seconds)"),
...
def render_invalid_reason_heatmap(df_attempts: pd.DataFrame, username: str, level_filter):
    ...
    reasons = [
        ...
        "equation_parse_failed",
        "operations_mismatch",
        "no_change",
        "not_equivalent_to_current",
    ]

    pivot = (
        df.assign(reason=df["invalid_reason_norm"].replace("", "other"))
        .pivot_table(
            index="level_label",
            columns="reason",
            values="attempt_id",
...
# page entry point
# -----------------------------
def show_solving_equations_reports():
    st.title("Solving Equations Reports")
    st.caption("Analytics from solving_equations_attempts")

    try:
        df_attempts = load_solving_equations_attempts_df()
    except Exception as e:
        st.error(f"Could not load solving-equations attempts: {e}")
        return

    if df_attempts.empty:
        st.warning("No solving-equations attempt data found in solving_equations_attempts.")
        return

    students = sorted(
        [u for u in df_attempts["student_label"].dropna().unique().tolist() if str(u).strip()]
    )
    ...
    with c1:
        username = st.selectbox("Student", student_options, key="se_report_student")

    with c2:
        level_filter = st.selectbox("Level", ["All"] + levels, key="se_report_level")

    with c3:
        min_questions = st.number_input(
            "Min Q", min_value=1, max_value=100, value=1, step=1, key="se_report_min_q"
        )
    ...
    with c4:
        metric = st.selectbox(
            ...
            [
                ...
                "hints_used",
                "parse_errors",
                "non_equivalent_steps",
            ],
            key="se_report_metric",
        )
